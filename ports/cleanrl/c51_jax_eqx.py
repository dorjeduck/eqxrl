...
"""
    ...
    def update(q_state, observations, actions, next_observations, rewards, dones):
        next_pmfs = forward_batch(
            q_state.target_model, next_observations
        ...
        (loss_value, old_values), grads = jax.value_and_grad(loss, has_aux=True)(
            q_state.model, observations, actions, target_pmfs
        )
    ...
    for global_step in range(args.total_timesteps):
        ...
        if random.random() < epsilon:
            ...
            pmfs = forward_batch(q_state.model, obs)
            ...
            if global_step % args.target_network_frequency == 0:
                q_state = q_state.replace(
                    target_model=optax.incremental_update(
                        q_state.model, q_state.target_model, 1
                    )
                )
    """
    if args.save_model:
